...
async fn main() -> Result<()> {
    ...
    info!(
        ...
        "starting nanotrace UDP client"
    );

    let udp_task = tokio::spawn(receive_udp(Arc::clone(&cfg), Arc::clone(&metrics), tx));
...
impl Config {
    fn from_env() -> Result<Self> {
        let bind = env_or("NANOTRACE_CLIENT_BIND", "127.0.0.1:4319")
            .parse()
...
async fn receive_udp(
    ...
    let socket = UdpSocket::bind(cfg.bind)
        .await
        .with_context(|| format!("bind UDP socket {}", cfg.bind))?;
    ...
    loop {
        ...
        for event in expand_events(value) {
            let Some(event) = enrich_event(event, &cfg.enrichment) else {
                metrics
                    .events_dropped_invalid
                    .fetch_add(1, Ordering::Relaxed);
                warn!(%peer, "dropping datagram item that does not match event object shape");
                continue;
            };
